''' Helper script for the pyManga.json notation.
    def parse_jsontemplate: returns a python dictionary with the resolved dependencies.
...
def option_resolveReferences(option,optionMap,baseOptions):
    ...
    if option["typedict"]["type"] == "complex":
        for subOption in option["value"]:
            subOption = option_resolveReferences(subOption,optionMap,baseOptions)
...
def fqn(option):
    namespace = "{}:".format(option["$namespace"]) if "$namespace" in option else ""
...
def buildNode(option):
    ''' Transform option in xml tree.
        ...
        option.value becomes element entry if the option is of type "integer", "float" or "string".
        option.value holds suboptions if option is of type "complex"
        if option is of type "alternative", option.value is the index of the chosen alternative.
    '''
    name = option["name"]    
    if option["typedict"]["type"] == "complex":
