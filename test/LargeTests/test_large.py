...
filepathExampleSetups = path.join(path.dirname(path.abspath(__file__)),"Test_Setups_large/*.xml")
xml = glob.glob(filepathExampleSetups)
errors = []
if xml:
    for xmlfile in xml:
        resultsFilepath = path.join(path.dirname(path.abspath(__file__)),"Test_Setups_large/testoutputs/*.*")
